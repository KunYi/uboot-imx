...
#define LCD_PWR_EN	IMX_GPIO_NR(5, 6)
#define LCD_RESET	IMX_GPIO_NR(2, 16)
#define LCD_BL		IMX_GPIO_NR(2, 10)
...
#define IT6251_I2C_BUS					(0) /* due to only enabled I2C3 */
#define IT6251_I2C_ADDR					(0x5D)
static int it6251_is_stable(void)
{
	const unsigned int caddr = IT6251_I2C_ADDR;
	const unsigned int laddr = IT6251_I2C_ADDR+1;
	int status;
	int clkcnt;
	int rpclkcnt;
	int refstate;
...
static int detect_lvds(struct display_info_t const *dev)
{
	...
	ret = i2c_set_bus_num(IT6251_I2C_BUS);
	if (ret) {
